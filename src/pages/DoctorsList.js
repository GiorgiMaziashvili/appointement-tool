  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">ჩვენი ექიმები</h1>
          <p className="mt-1 text-sm text-gray-600">
            იპოვეთ და დაჯავშნეთ ვიზიტები ჩვენი კვალიფიციური სამედიცინო სპეციალისტებთან.
          </p>
        </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
          <h2 className="text-lg font-medium text-gray-900 mb-2 sm:mb-0">ექიმების ფილტრი</h2>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="available">მხოლოდ ხელმისაწვდომი</option>
              <option value="busy">მხოლოდ დაკავებული</option>
            </select>
          </div>
          ...
          <div className="flex items-end">
            <div className="text-sm text-gray-600">
              <span className="font-medium">{filteredDoctors.length}</span> ექიმი ნაპოვნია
            </div>
          </div>
        </div>
      </div>
      ...
      {filteredDoctors.length > 0 ? (
        ...
        <div className="bg-white shadow-md rounded-lg p-12 text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">ექიმები არ მოიძებნა</h3>
          <p className="mt-1 text-sm text-gray-500">
            თქვენი ამჟამინდელი ფილტრის კრიტერიუმებს არცერთი ექიმი არ შეესაბამება. სცადეთ ფილტრების რეგულირება ან გასუფთავება.
          </p>
          <div className="mt-6">
            <button
              onClick={clearFilters}
      ...
      <div className="bg-primary-50 border border-primary-200 rounded-lg p-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-sm text-primary-700">სულ ექიმები</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-green-600">
              {doctors.filter(d => d.available).length}
            </div>
            <div className="text-sm text-green-700">ახლა ხელმისაწვდომი</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-primary-600">{specialties.length}</div>
            <div className="text-sm text-primary-700">სპეციალობები</div>
          </div>
        </div>
      </div>
    </div>
